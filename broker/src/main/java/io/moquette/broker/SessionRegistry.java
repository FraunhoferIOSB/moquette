...
public class SessionRegistry {
    ...
    private void reactivateSubscriptions(Session session, String username) {
        ...
        for (Subscription existingSub : session.getSubscriptions()) {
            final boolean topicReadable = authorizator.canRead(existingSub.getTopicFilterClient(), username,
                session.getClientID());
            if (!topicReadable) {
                subscriptionsDirectory.removeSubscription(existingSub.getTopicFilterClient(), session.getClientID());
            }
            // TODO
//            subscriptionsDirectory.reactivate(existingSub.getTopicFilterClient(), session.getClientID());
        }
    }
